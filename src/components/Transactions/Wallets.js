...
import WalletsTransactionTable from "./WalletsTransactionTable";

const WalletsTrasactions = () => {
  ...
  const fetchData = (page, params, load) => {
    ...
    TransactionServices.getWalletsPayments(page||0, params)
    .then(res=>{
      setLoading(false)
      console.log(res)
      setData(res.data)
...
//   }
  ...
  return (
    <>
      <PageTitle>{`Transactions - Wallets Payments`}</PageTitle>
      ...
      <Card className="min-w-0 shadow-xs overflow-hidden bg-white dark:bg-gray-800 mb-5">
        <CardBody>
            <div className="w-full flex items-center border border-gray-500 rounded-lg px-4">
              <input value={search} onChange={e=>setSearch(e.target.value)} type="text" placeholder="Search by transaction's channel, amount, type or status" 
      ...
      {loading ? (
        ...
        <TableContainer className="mb-8">
          <Table>
            ...
            <WalletsTransactionTable lang={lang} isCheck={isCheck} coupons={display} setIsCheck={setIsCheck} fetchData={()=>fetchData()} />
          </Table>
          <TableFooter>
            <Pagination
...
export default WalletsTrasactions;
