...
const Transactions = () => {
  ...
  const fetchData = (page, params, load) => {
    ...
    GiftCardServices.getAll(page||0, params)
    .then(res=>{
      setLoading(false)
      console.log(res)
      setData(res.data)
  ...
  return (
    <>
      <PageTitle>{`Transactions${params.type?` - ${transactionType[params.type]}`:` - Orders`}`}</PageTitle>
      ...
      {loading ? (
        ...
        <TableContainer className="mb-8">
          <Table>
            ...
            <GiftCardTable lang={lang} isCheck={isCheck} coupons={display} setIsCheck={setIsCheck} fetchData={()=>fetchData()} />
          </Table>
          <TableFooter>
            <Pagination
